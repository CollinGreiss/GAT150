...
namespace kiko {
	...
	public:
		...
		Matrix22() = default;
		Matrix22(const vec2& row1, const vec2& row2) { rows[0] = row1; rows[1] = row2; };
	...
	inline Matrix22 Matrix22::CreateIdentity() { return { {1, 0}, {0, 1} }; }

	inline Matrix22 Matrix22::CreateScale(const vec2& scale) { return { {scale.x, 0.0f}, { 0.0f, scale.y} }; }
	inline Matrix22 Matrix22::CreateScale(float scale) { return { {scale,   0.0f}, { 0.0f, scale  } }; }
